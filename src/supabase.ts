...
export async function fetchWPPoints(student_name: string): Promise<number> {
  const { data } = await supabase.from("wp_progress")
    .select("points").eq("student_name", student_name).maybeSingle();
  return data?.points ?? 0;
}

export async function upsertWPPoints(student_name: string, newPoints: number): Promise<void> {
  await supabase.from("wp_progress")
    .upsert({ student_name, points: newPoints }, { onConflict: "student_name" });
}

export async function fetchQFPoints(student_name: string): Promise<number> {
  const { data } = await supabase.from("qf_progress")
    .select("points").eq("student_name", student_name).maybeSingle();
  return data?.points ?? 0;
}

export async function upsertQFPoints(student_name: string, newPoints: number): Promise<void> {
  await supabase.from("qf_progress")
    .upsert({ student_name, points: newPoints }, { onConflict: "student_name" });
}
...
export async function upsertFactPoints(student_name: string, newPoints: number): Promise<void> {
  await supabase.from("factoring_progress")
    .upsert({ student_name, points: newPoints }, { onConflict: "student_name" });
}
